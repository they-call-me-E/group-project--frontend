...
const AddMembers = ({
  ...
  setAddMemberSuccess: React.Dispatch<React.SetStateAction<any>>;
  setAddMembererror: React.Dispatch<React.SetStateAction<any>>;
  reFetchGroupListData: () => void;
  setaddMemberErrorMsg: React.Dispatch<React.SetStateAction<any>>;
}) => {
  ...
  const handleAddMember = (userId: any, groupId: string) => {
    axios
      .patch(
        // @ts-ignore
        `${process.env.NEXT_PUBLIC_API_URL}/groups/${groupId}/add/members`,
        {
          userId: userId,
        },
        {
      ...
      .then((response) => {
        if (response?.data?.document?.members?.length > 0) {
      ...
      .catch((error) => {
        if (error?.response?.data?.message) {
          ...
          setaddMemberErrorMsg(error?.response?.data?.message);
        }
      });
  };
      ...
      >
        ...
        <Grid sx={{ display: "flex", columnGap: "6px", alignItems: "center" }}>
          ...
          <Button
            ...
            variant="contained"
            onClick={() => {
              handleAddMember(userId, editGroupInformation?.uuid);
            }}
