...
import { handleGroupMembersList } from "./../../../utils/api/groupInformation";
...
const RemoveMembers = ({
  ...
  setRemoveMemberSuccess: React.Dispatch<React.SetStateAction<any>>;
  setRemoveMembererror: React.Dispatch<React.SetStateAction<any>>;
  reFetchGroupListData: () => void;
  setRemoveMemberErrorMsg: React.Dispatch<React.SetStateAction<any>>;
}) => {
  ...
  useEffect(() => {
    // @ts-ignore
    handleGroupMembersList(session?.user?.token, editGroupInformation?.uuid)
      .then((res) => {
        setUserList((prev: any) => {
          return res?.data?.document?.members?.map((item: any) => {
            return {
              uuid: item?.uuid,
              label: item?.name,
              avatar: item?.avatar,
  ...
  const handleRemoveMember = (userId: any, groupId: string) => {
    axios
      .patch(
        // @ts-ignore
        `${process.env.NEXT_PUBLIC_API_URL}/groups/${groupId}/remove/members`,
        {
          userId: userId,
        },
        {
      ...
      .then((response) => {
        if (response?.data?.document?.members) {
          setRemoveMemberSuccess(true);
          setOpenRemoveMembersModal(false);
          reFetchGroupListData();
        }
      })
      .catch((error) => {
        if (error?.response?.data?.message) {
          setRemoveMembererror(true);
          setRemoveMemberErrorMsg(error?.response?.data?.message);
        }
      });
  };
      ...
      >
        ...
        <Grid sx={{ display: "flex", columnGap: "6px", alignItems: "center" }}>
          ...
          <Button
            ...
            variant="contained"
            onClick={() => {
              handleRemoveMember(userId, editGroupInformation?.uuid);
            }}
